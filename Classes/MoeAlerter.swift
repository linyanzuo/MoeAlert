...
extension Alerter {
    ...
    public class func show(
        ...
        var optionalAlertView: MoeAlertView? = nil
        // 1. 若父视图中已存在AlertView实例，则直接置顶使用
        for subview in view.subviews {
            if let subAlertView = subview as? MoeAlertView {
        ...
        if optionalAlertView == nil {
            optionalAlertView = MoeAlertView(frame: view.bounds)
    ...
    public class func hide(in view: UIView, with identifier: String) {
        checkThread()
        for subview in view.subviews {
            if let alertView = subview as? MoeAlertView {
