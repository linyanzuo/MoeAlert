...
open class MoeAlertView: UIView {
    /// 是否启用动画
...
//    }
    ...
    private(set) lazy var maskBtn: UIButton = {
        ...
        btn.alpha = 1.0
        btn.backgroundColor = UIColor(red: 48, green: 48, blue: 48, alpha: 1.0)
        btn.addTarget(self, action: #selector(self.maskTapAction(_:)), for: .touchUpInside)
        return btn
    }()
}
...
//        debugPrint("新增提示")
        ...
        if useAnaimation == true {
            debugPrint("Frmae of AlertView's SuperView: \(self.superview!.frame)")
...
//        debugPrint("移除提示")
        ...
        guard alerts.count > 0 else { debugPrint("没有正在展示的提示"); return }
        ...
        if identifier != nil {
            ...
            if targetAlert == nil {
                debugPrint("找不到与指定标识\(identifier!)匹配的提示")
                return
            }
        }
        // 3. 判断遮罩按钮是否移除
...
// MARK: - 事件响应
@objc extension MoeAlertView {
    @objc func maskTapAction(_ sender: UIButton) {
        debugPrint("【AlertView】点击了遮罩按钮")
        // 启用点击遮罩关闭弹窗时，移除弹窗
...
// MARK: - 基础动画
extension MoeAlertView: CAAnimationDelegate {
...
//        case external
    ...
    public func animationDidStart(_ anim: CAAnimation) {
        ...
        debugPrint("【AlertView】动画：\(animName)，过渡类型：\(transitionType)，开始执行")
    }
    ...
    public func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        ...
        debugPrint("【AlertView】动画：\(animName)，过渡类型：\(transitionType)，执行结束")
